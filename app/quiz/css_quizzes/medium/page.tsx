...
/**
 * Medium CSS Quiz Page
 * Displays 10 medium-level CSS quiz questions
 */
export default function MediumCssQuiz() {
    const { user } = useAuth();
    ...
    useEffect(() => {
        const fetchAll = async () => {
            ...
            try {
                ...
                const response = await fetch('/api/quiz?type=css&difficulty=medium');
                ...
                if (data.success) {
                    ...
                    if (user?.id) {
                        const res = await fetch(`/api/user-answers?user_id=${user.id}&quiz_type=css&difficulty=medium`);
                        if (res.ok) {
        ...
        <main className="px-10 mt-[130px] mb-[200px] flex justify-start gap-24">
            ...
            <div className="w-[600px]">
                <div className="flex justify-between items-center mb-6">
                    <Link href="/quiz/css_quizzes" className="text-(--secondary-color) font-bold">
                        ← Back
                    </Link>
                    <span className="text-yellow-500 font-semibold">CSS Medium Level</span>
                </div>
