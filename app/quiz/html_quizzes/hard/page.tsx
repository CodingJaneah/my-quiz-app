...
/**
 * Hard HTML Quiz Page
 * Displays 10 hard-level HTML quiz questions
 */
export default function HardHtmlQuiz() {
    const { user } = useAuth();
    ...
    useEffect(() => {
        const fetchAll = async () => {
            ...
            try {
                ...
                const response = await fetch('/api/quiz?type=html&difficulty=hard');
                ...
                if (data.success) {
                    ...
                    if (user?.id) {
                        const res = await fetch(`/api/user-answers?user_id=${user.id}&quiz_type=html&difficulty=hard`);
                        if (res.ok) {
    ...
    return (
        <>
        <Header />
        <main className="px-10 mt-[130px] mb-[200px] pb-24 flex justify-start gap-24">

                {/* Question Navigation Dots */}
            <div className="flex justify-center gap-2 mt-40 flex-wrap">
            ...
            <div className="w-[600px]">
                <div className="flex justify-between items-center mb-6">
                    <Link href="/quiz/html_quizzes" className="text-(--secondary-color) font-bold">
                        ← Back
                    </Link>
                    <span className="text-red-500 font-semibold">HTML Hard Level</span>
                </div>
